"""Detect process-wide atexit hook registration in production modules."""
import argparse
...
def audit_source(source):
 return [f"atexit.register() mutates process shutdown behavior on line {c.lineno}" for c in iter_calls(source) if call_name(c)=="atexit.register"]
def audit(root):
 root=require_root(root); out=[]
 for rel in production_python_files(root):
  try: src=(root/rel).read_text(encoding="utf-8")
  except (OSError,UnicodeDecodeError): continue
  out += [f"{rel}: {x}" for x in audit_source(src)]
 return out
def main(argv=None):
