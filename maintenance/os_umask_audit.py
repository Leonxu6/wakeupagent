"""Detect os.umask() calls that mutate process-wide file creation policy."""
...
def audit_source(source: str)->list[str]:
    ...
    return [f"os.umask() mutates process-wide permissions on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="os" and n.func.attr=="umask"]
