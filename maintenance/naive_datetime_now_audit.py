"""Detect datetime.now() calls without an explicit timezone."""
...
def audit_source(source: str)->list[str]:
    ...
    for n in ast.walk(tree):
        if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and n.func.attr=="now" and isinstance(n.func.value,ast.Name) and n.func.value.id=="datetime" and not n.args and not any(k.arg in {"tz","timezone"} for k in n.keywords): out.append(f"datetime.now() without timezone on line {n.lineno}")
    return out
