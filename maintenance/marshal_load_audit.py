"""Detect marshal.load/loads calls that deserialize implementation-specific bytecode data."""
...
_NAMES={"marshal.load","marshal.loads"}

def audit_source(source:str)->list[str]:
    return [f"{call_name(c)} deserializes marshal data on line {c.lineno}" for c in iter_calls(source) if call_name(c) in _NAMES]
