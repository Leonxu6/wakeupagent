...
def audit_source(source:str)->list[str]:
    ...
    for c in iter_calls(source):
        ...
        for kw in c.keywords:
            if kw.arg=="preexec_fn" and not (isinstance(kw.value,ast.Constant) and kw.value.value is None): out.append(f"subprocess preexec_fn is unsafe in threaded services on line {c.lineno}")
    return out
