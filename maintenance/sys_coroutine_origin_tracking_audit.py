"""Detect process-wide coroutine origin tracking depth changes."""
import argparse
...
def audit_source(s): return [f"sys.set_coroutine_origin_tracking_depth() mutates process diagnostics on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="sys.set_coroutine_origin_tracking_depth"]
def audit(root):
