"""Detect shelve persistence, which implicitly uses pickle serialization."""
import argparse,ast
...
def audit_source(s):
 ...
 return [f"shelve.open() uses pickle-backed persistence on line {n.lineno}" for n in ast.walk(t) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="shelve" and n.func.attr=="open"]
def audit(root):
