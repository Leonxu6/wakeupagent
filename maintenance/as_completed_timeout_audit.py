...
def audit_source(source:str)->list[str]:
    return [f"as_completed without timeout on line {c.lineno}" for c in iter_calls(source) if call_name(c) in {"concurrent.futures.as_completed","futures.as_completed"} and len(c.args)<2 and not has_keyword(c,"timeout")]
