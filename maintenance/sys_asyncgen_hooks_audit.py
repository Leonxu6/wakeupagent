"""Detect process-wide asynchronous generator hook mutation."""
import argparse
...
def audit_source(s): return [f"sys.set_asyncgen_hooks() mutates process async-generator behavior on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="sys.set_asyncgen_hooks"]
def audit(root):
