"""Detect asyncio.wait() calls without explicit timeout bounds."""
...
def audit_source(source:str)->list[str]:
    return [f"asyncio.wait without timeout on line {c.lineno}" for c in iter_calls(source) if call_name(c)=="asyncio.wait" and not has_keyword(c,"timeout")]
