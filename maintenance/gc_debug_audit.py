"""Detect gc.set_debug() process-wide GC diagnostic changes."""
import argparse
...
def audit_source(s): return [f"gc.set_debug() mutates process-wide GC diagnostics on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="gc.set_debug"]
def audit(root):
