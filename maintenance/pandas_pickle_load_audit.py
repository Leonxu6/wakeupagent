"""Detect pandas.read_pickle() calls that deserialize pickle-backed data."""
...
def audit_source(source:str)->list[str]:
    return [f"{call_name(c)} deserializes pickle data on line {c.lineno}" for c in iter_calls(source) if call_name(c) in {"pandas.read_pickle","pd.read_pickle"}]
