"""Detect os.putenv()/os.unsetenv() process-wide mutations."""
import argparse,ast
...
def audit_source(s):
 ...
 return [f"os.{n.func.attr}() mutates process environment on line {n.lineno}" for n in ast.walk(t) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="os" and n.func.attr in {"putenv","unsetenv"}]
def audit(root):
