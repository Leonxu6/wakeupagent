"""Detect sqlite3.enable_shared_cache() global connection-policy changes."""
import argparse
...
def audit_source(s): return [f"sqlite3.enable_shared_cache() mutates global SQLite connection policy on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="sqlite3.enable_shared_cache"]
def audit(root):
