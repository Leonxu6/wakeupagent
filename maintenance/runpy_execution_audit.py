"""Detect runpy APIs that execute modules or files dynamically."""
...
_NAMES={"runpy.run_module","runpy.run_path"}

def audit_source(source: str) -> list[str]:
    return [f"{call_name(c)} dynamically executes Python code on line {c.lineno}" for c in iter_calls(source) if call_name(c) in _NAMES]
