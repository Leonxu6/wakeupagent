"""Detect signal.signal() calls that mutate process-wide signal handling."""
...
def audit_source(source: str)->list[str]:
    ...
    return [f"signal.signal() mutates process-wide handlers on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="signal" and n.func.attr=="signal"]
