...
def audit_source(source: str) -> list[str]:
    ...
    for c in iter_calls(source):
        if call_name(c)=="select.select" and (len(c.args)<4 or (isinstance(c.args[3],ast.Constant) and c.args[3].value is None)):
            out.append(f"select.select without finite timeout on line {c.lineno}")
    return out
