"""Detect process-wide multiprocessing start-method mutation."""
import argparse
...
def audit_source(s): return [f"multiprocessing.set_start_method() mutates process policy on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="multiprocessing.set_start_method"]
def audit(root):
