"""Detect process-wide forkserver preload configuration."""
import argparse
...
def audit_source(s): return [f"multiprocessing.set_forkserver_preload() mutates child import policy on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="multiprocessing.set_forkserver_preload"]
def audit(root):
