"""Detect gc.disable() calls that disable cyclic collection process-wide."""
...
def audit_source(source: str)->list[str]:
    ...
    return [f"gc.disable() disables cyclic collection process-wide on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="gc" and n.func.attr=="disable"]
