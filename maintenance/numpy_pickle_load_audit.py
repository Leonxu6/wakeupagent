...
def audit_source(source: str) -> list[str]:
    ...
    for c in iter_calls(source):
        ...
        if any(k.arg=="allow_pickle" and isinstance(k.value,ast.Constant) and k.value.value is True for k in c.keywords):
            out.append(f"{call_name(c)} enables pickle deserialization on line {c.lineno}")
    return out
