"""Detect gc.set_threshold() process-wide collection-policy changes."""
import argparse
...
def audit_source(s): return [f"gc.set_threshold() mutates process-wide collection policy on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="gc.set_threshold"]
def audit(root):
