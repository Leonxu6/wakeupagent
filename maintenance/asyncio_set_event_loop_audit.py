"""Detect asyncio.set_event_loop() calls that replace thread-local loop state."""
import argparse
...
def audit_source(s): return [f"asyncio.set_event_loop() mutates thread-local loop state on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="asyncio.set_event_loop"]
def audit(root):
