"""Detect os.forkpty() calls that fork the agent behind a pseudo-terminal."""
...
def audit_source(source:str)->list[str]:
    return [f"os.forkpty requires explicit process and terminal lifecycle review on line {c.lineno}" for c in iter_calls(source) if call_name(c)=="os.forkpty"]
