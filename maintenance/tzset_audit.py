"""Detect time.tzset(), which mutates process-local timezone behavior."""
import argparse,ast
...
def audit_source(s):
 ...
 return [f"time.tzset() mutates process timezone on line {n.lineno}" for n in ast.walk(t) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="time" and n.func.attr=="tzset"]
def audit(root):
