"""Detect logging.captureWarnings() process-wide warning-routing changes."""
import argparse
...
def audit_source(s): return [f"logging.captureWarnings() mutates process warning routing on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="logging.captureWarnings"]
def audit(root):
