"""Detect socket.setdefaulttimeout() calls that mutate networking defaults process-wide."""
...
def audit_source(source: str)->list[str]:
    ...
    return [f"socket.setdefaulttimeout() mutates process-wide networking on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="socket" and n.func.attr=="setdefaulttimeout"]
