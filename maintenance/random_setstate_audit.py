"""Detect random.setstate() module-global RNG replacement."""
import argparse
...
def audit_source(s): return [f"random.setstate() replaces module-global RNG state on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="random.setstate"]
def audit(root):
