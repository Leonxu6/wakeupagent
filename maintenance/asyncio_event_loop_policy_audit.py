"""Detect process-wide asyncio event-loop policy mutation."""
import argparse
...
def audit_source(s): return [f"asyncio.set_event_loop_policy() mutates process async policy on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="asyncio.set_event_loop_policy"]
def audit(root):
