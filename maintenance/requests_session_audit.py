"""Detect requests.Session() construction that needs explicit ownership/cleanup."""
import argparse,ast
...
def audit_source(s):
 ...
 return [f"requests.Session() needs explicit lifecycle on line {n.lineno}" for n in ast.walk(t) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="requests" and n.func.attr=="Session"]
def audit(root):
