"""Detect sys.setrecursionlimit() calls that mutate interpreter-wide recursion policy."""
...
def audit_source(source: str)->list[str]:
    ...
    return [f"sys.setrecursionlimit() mutates interpreter-wide state on line {n.lineno}" for n in ast.walk(tree) if isinstance(n,ast.Call) and isinstance(n.func,ast.Attribute) and isinstance(n.func.value,ast.Name) and n.func.value.id=="sys" and n.func.attr=="setrecursionlimit"]
