"""Detect profiling hooks installed across all Python threads."""
import argparse
...
def audit_source(s): return [f"threading.setprofile_all_threads() mutates global profiling behavior on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="threading.setprofile_all_threads"]
def audit(root):
