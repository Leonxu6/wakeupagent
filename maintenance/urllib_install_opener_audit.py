...
def audit_source(s): return [f"urllib.request.install_opener() mutates global URL handling on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="urllib.request.install_opener"]
def audit(root):
