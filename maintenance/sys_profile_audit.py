"""Detect sys.setprofile() runtime profiling hooks."""
import argparse
...
def audit_source(s): return [f"sys.setprofile() installs a runtime profiling hook on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="sys.setprofile"]
def audit(root):
