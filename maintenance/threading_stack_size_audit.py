"""Detect process-wide threading.stack_size() mutation."""
import argparse
...
def audit_source(s): return [f"threading.stack_size() mutates the default thread stack on line {c.lineno}" for c in iter_calls(s) if call_name(c)=="threading.stack_size" and c.args]
def audit(root):
