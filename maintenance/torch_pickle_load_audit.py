...
def audit_source(source:str)->list[str]:
    ...
    for c in iter_calls(source):
        ...
        for kw in c.keywords:
            if kw.arg=="weights_only" and isinstance(kw.value,ast.Constant) and kw.value.value is False: out.append(f"torch.load disables weights-only loading on line {c.lineno}")
    return out
