...
def audit_source(source:str)->list[str]:
    ...
    for c in iter_calls(source):
        name=call_name(c) or ""
        if name.endswith(".load_extension"): out.append(f"SQLite load_extension executes native code on line {c.lineno}")
        elif name.endswith(".enable_load_extension") and c.args and isinstance(c.args[0],ast.Constant) and c.args[0].value is True: out.append(f"SQLite extension loading enabled on line {c.lineno}")
    return out
