...
def audit_source(s):
 ...
 for n in iter_assignments(s):
  if any(name.startswith("decimal.DefaultContext.") for name in assignment_targets(n)):o.append(f"decimal.DefaultContext mutation changes process Decimal defaults on line {n.lineno}")
 return o
def audit(root):
