"""Detect direct os.fork() calls in long-running agent modules."""
...
def audit_source(source: str) -> list[str]:
    return [f"os.fork requires explicit lifecycle review on line {c.lineno}" for c in iter_calls(source) if call_name(c)=="os.fork"]
